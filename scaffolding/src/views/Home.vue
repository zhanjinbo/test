<template>
  ...
  <div style="padding:20px;background-color:#0aa1ed;color:#fff">我是底线</div>
  </div>
</template>
<style scoped>
/* 因为roturelink翻译成了a标签 , 所以这个要写a */
.shortcut a {
  color: #fff;
  padding-left: 20px;
}
...
export default {
  ...
  watch: {
    ...
    active(value) {
      ...
      // console.log(typeof this.active);//string
    },
  ...
  mounted() {
    ...
    // 调用获取数据的自定义函数 - loadData();
    // this.loadData(this.active, this.page);
    // console.log(this.moment.unix(0).format());//1970-01-01T08:00:00+08:00
    // console.log(this.moment.unix(0).hour(15).format());//1970-01-01T15:00:00+08:00
    // console.log(this.moment.unix(0).hour(15).format('Y年'));//1970年
    // console.log(this.moment.unix(0).hour(15).format('Y年'));//1970年
    // console.log(this.moment.unix(0).hour(15).format('Y年MM月DD日'));//1970年1月1日
    // console.log(this.moment.unix(0).hour(15).format('Y年MM月DD日HH:mm'));//1970年1月1日15:00
    this.loadData(this.active,this.page);
  },
};
</script>